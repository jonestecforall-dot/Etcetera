...
func _ready() -> void:
	...
	_fade_in_from_transition()


# Fades out the persistent SceneFadeLayer that the title screen left opaque.
# Without this, you'd arrive on a black screen forever.
func _fade_in_from_transition(fade_duration : float = 0.4) -> void:
	var fade_layer := get_tree().root.get_node_or_null("SceneFadeLayer")
	if fade_layer == null:
		return
	var rect := fade_layer.get_node_or_null("FadeRect") as ColorRect
	if rect == null:
		return
	
	# If it isn't opaque, nothing to fade — bail (avoids stray tweens)
	if rect.modulate.a <= 0.01:
		return
	
	# Wait one frame so the menu has actually painted underneath before we reveal it
	await get_tree().process_frame
	
	var t := create_tween()
	t.tween_property(rect, "modulate:a", 0.0, fade_duration)\
...
func _build_ui() -> void:
	...
	# Level dropdown — now full-width, since LOAD is gone
...
@export_file("*.tscn") var title_screen_scene_path : String = "res://title_screen.tscn"

func _on_quit_pressed() -> void:
	print("↩ Returning to title screen...")
	if title_screen_scene_path == "" or not ResourceLoader.exists(title_screen_scene_path):
		push_warning("IntroMenu: title_screen_scene_path is not set or doesn't exist — falling back to quit.")
		get_tree().quit()
		return
