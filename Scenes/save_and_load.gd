...
func _ensure_save_folder() -> void:
	...
	if not dir.dir_exists(editor.save_folder):
		dir.make_dir(editor.save_folder)
		print("📁 Created save folder: ", editor.save_folder)

# 🆕 Ensure res://worlds/ exists inside the project.
...
func _create_save_dialog() -> void:
	...
	save_dialog.size = Vector2(540, 620)   # 🆕 taller for the new button
...
func _refresh_save_list() -> void:
	...
	# Personal
	var dir = DirAccess.open(editor.save_folder)
	if dir:
		dir.list_dir_begin()
	...
	if not personal_saves.is_empty():
		...
		for save_file in personal_saves:
			_add_save_row(save_file, editor.save_folder, false)
...
func _on_save_new_pressed() -> void:
	...
	var full_path = editor.save_folder + level_name
...
func _on_load_pressed() -> void:
	...
	# Try personal first, fall back to shared.
	var full_path : String = editor.save_folder + level_name
	if not FileAccess.file_exists(full_path):
		var shared_path : String = SHARED_WORLDS_FOLDER + level_name
		if FileAccess.file_exists(shared_path):
			full_path = shared_path
		else:
			print("❌ Level not found in either folder: ", level_name)
			return
...
# 🆕 EXPORT TO PROJECT — copies the current personal save into res://worlds/
# 🆕 EXPORT TO PROJECT — copies the current personal save into res://worlds/
func _on_export_to_project_pressed() -> void:
	...
	var source_path : String = editor.save_folder + level_name
	if not FileAccess.file_exists(source_path):
		print("⚠️ Cannot export: no personal save named ", level_name)
		print("   (save the world first, then export it)")
		return
...
func _on_delete_all_saves_confirmed() -> void:
	var dir = DirAccess.open(editor.save_folder)
	if dir == null:
		print("❌ Could not open save folder: ", editor.save_folder)
		return
...
func _duplicate_enemy_for_save(enemy: Node) -> Node:
	...
	# Try meta tag first (stamped by BuildEditor._place_enemy_at)
	...
	if scene_index >= 0 and scene_index < editor.enemy_scenes.size():
		...
		# Fallback: use the scene's own file path
	...
	# Preserve name so the tree stays readable
	copy.name = enemy.name
	
	# Re-stamp the meta so load_world() can identify it as an enemy
...
func load_world(path: String) -> void:
	...
	for child in children_to_process:
		...
		if child is StaticBody2D:
			...
			# 👾 Enemy vs 🐚 Shell — disambiguate by meta tag / group
			if child.has_meta("enemy_scene_index") or child.is_in_group("enemy"):
				loaded_level.remove_child(child)
				editor.tile_container.add_child(child)
				editor.placed_enemies.append(child)
			elif child is Node and child.get_script() != null:
				loaded_level.remove_child(child)
				editor.tile_container.add_child(child)
...
func set_save_target(target: int) -> void:
	if editor == null:
		push_warning("save_and_load: set_save_target called before setup()")
		return
	_save_target = target
	editor.save_folder = GITHUB_SAVE_FOLDER if target == SaveTarget.GITHUB_EXPORT else NORMAL_SAVE_FOLDER
	_ensure_save_folder()
	print("💾 save_and_load folder → ", editor.save_folder)
