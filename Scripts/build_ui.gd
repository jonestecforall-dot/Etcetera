...
func _build_ui_elements() -> void:
	...
	# --- Framed panel behind the top-right controls ---
	_panel = PanelContainer.new()
	_panel.set_anchors_preset(Control.PRESET_TOP_RIGHT)
	_panel.position = Vector2(-14, 14) # Offsets from the top-right corner
	...
	scroll.size_flags_vertical = Control.SIZE_FILL # Fixed height
	...
	tile_grid.size_flags_vertical = Control.SIZE_SHRINK_CENTER
	scroll.add_child(tile_grid)

	_pan_grayed_controls.append(scroll)

	# --- EXTRA CONTROLS (Zoom + Pan) — BOTTOM OF PANEL ---
	main_container.add_child(_make_divider())

	var extra_controls_row := HBoxContainer.new()
	extra_controls_row.add_theme_constant_override("separation", 10)
	extra_controls_row.alignment = BoxContainer.ALIGNMENT_CENTER
	main_container.add_child(extra_controls_row)

	# Build them inside the row instead of absolute positions
	_build_zoom_buttons(extra_controls_row)
	_build_pan_button(extra_controls_row)
# ==========================================
# SMALL UI HELPERS
# ==========================================
...
# ==========================================
# BUILD ZOOM BUTTONS (Now accepts a container)
# ==========================================
func _build_zoom_buttons(parent_container: HBoxContainer) -> void:
	var zbtn_size := 60.0
	...
	parent_container.add_child(zoom_in_btn)
	...
	parent_container.add_child(zoom_out_btn)
...
# ==========================================
# PAN BUTTON (Now accepts a container)
# ==========================================
func _build_pan_button(parent_container: HBoxContainer) -> void:
	pan_button = Button.new()
	pan_button.text = "✋  PAN"
	pan_button.custom_minimum_size = Vector2(110, 60)
	...
	parent_container.add_child(pan_button)
...
# ==========================================
# INPUT BLOCKING FOR THE GRID
# ==========================================
# COPY THIS FUNCTION TO YOUR GRID SCRIPT
# and call it at the very top of your placement logic:
# if _is_mouse_over_ui(): return
func _is_mouse_over_ui(screen_pos: Vector2) -> bool:
	if _panel != null and _panel.get_global_rect().has_point(screen_pos):
		return true
	if pan_button != null and pan_button.get_global_rect().has_point(screen_pos):
		return true
	# Check if mouse is over any zoom button
	for btn in _pan_disabled_buttons:
		if is_instance_valid(btn) and btn.get_global_rect().has_point(screen_pos):
			if btn.visible:
				return true
	return false
